...
## canny
  ...
  >在设计完成后，您可以使用我们提供的canny_test.cpp文件进行基本功能测试。但是，由于在评分时会统一使用评分系统内部的canny.h和canny_test.cpp，因此请不要将设计写在canny.h和canny_test.cpp中，canny.h和canny_test.cpp文件也无需提交。如果您有修改头文件的需求，您可以在设计中添加文件名不为canny.h的头文件。
