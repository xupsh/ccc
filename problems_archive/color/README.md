...
## K-1 Coloring
  ...
  3）提交程序
  >您仅需提交cpp文件和h文件。请将除test.cpp以外的所有cpp和h文件压缩为一个压缩包color.zip，在页面下方您的解答处点击“上传代码”按钮进行提交，稍待片刻即可在文本框下方得知您的设计是否通过了基本功能测试。延迟、面积等性能指标则可在历史提交记录中点击版本号详细查看。
  
  4）评分规则
  >设计的最终得分 = color * color * 性能得分，最终得分越小，排名越高
  >
  >其中，color为设计使用color的数量，在结果正确的情况下越小越好。
  >性能得分的计算公式为：<br>
